...
void NodeTree::ImprimirARN(Nodo* N, int i)
{
	...
	ImprimirARN(N->Der, i + 1);
	ImprimirEspacios(i);
	cout << N->Valor << (N->Color == rojo ? " (rojo)" : " negro") << endl;
	ImprimirARN(N->Izq, i + 1);
}
...
int main()
{
	...
	vector <int> PreOrder = { 45,10,4,39,25,16,27,42,71,61,60,51,89,88,73 };
	vector <int> InOrder = { 4,10,16,25,27,39,42,45,51,60,61,71,73,88,89 };
	vector <int> PostOrder = { 4,16,27,25,42,39,10,51,60,61,73,88,89,71,45 };
